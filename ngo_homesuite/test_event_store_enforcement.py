...
def test_workflow_write_boundary_is_orchestrated_through_write_gate() -> None:
    ...
    assert "raise RuntimeError(\"Write outside WriteGate\")" in repo_text
    assert "raise RuntimeError(\"Write outside WriteGate\")" in store_text
...
def test_repository_save_rejects_direct_writes() -> None:
    ...
    with pytest.raises(RuntimeError, match="Write outside WriteGate"):
