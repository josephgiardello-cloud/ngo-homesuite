...
|---|---|---|---|
| Auth hardening (MFA capability) | PARTIAL | TOTP/backup-code/2FA enforcement and step-up endpoints exist in `ngo_homesuite/web/auth_routes.py`; encrypted-at-rest MFA secret handling and fail-closed tests are present in `ngo_homesuite/models/core.py` and `ngo_homesuite/models/test_mfa_secret_encryption.py`. Route-level step-up audit has been applied across high-risk destructive/export endpoints in `main_routes.py`, `reporting_routes.py`, `admin_grants_routes.py`, `program_routes.py`, `volunteer_routes.py`, and `v2_routes.py`. | Keep explicit operator documentation and key-management evidence current per release candidate. |
...
| Backup/restore + key rotation drills | PASS | Runbooks, migration preflight checks, and 2026-06-04 drill lane artifacts are present in `artifacts/release/backup-restore-drill-output.txt` and `artifacts/release/key-rotation-drill-output.txt`. | Keep per-candidate reruns attached. |
...
| Mobile/public form validation | PASS | 2026-06-04 mobile/public lane artifact is present in `artifacts/release/mobile-public-validation-output.txt`. | Keep per-candidate reruns attached. |
...
## Latest Validation Evidence (2026-06-04, Release Evidence Refresh)

Targeted release-evidence refresh lanes executed:

```bash
.venv\Scripts\python.exe -m pytest ngo_homesuite/web/test_auth_security_routes.py ngo_homesuite/ai/test_ai_hardening.py ngo_homesuite/db/test_connection_hardening.py -v --maxfail=10
.venv\Scripts\python.exe -m pytest ngo_homesuite/web/test_cross_tenant_boundaries.py ngo_homesuite/tests/test_tenant_isolation.py ngo_homesuite/tests/test_tenancy_service_guards.py -v --maxfail=10
.venv\Scripts\python.exe -m pytest ngo_homesuite/api/test_observability_api.py ngo_homesuite/api/test_observability_request_logs.py ngo_homesuite/test_observability_artifacts.py ngo_homesuite/web/test_health_endpoint.py -v --maxfail=10
.venv\Scripts\python.exe -m pytest ngo_homesuite/web/test_mobile_intake_routes.py ngo_homesuite/web/test_public_smoke_paths.py ngo_homesuite/web/test_public_stripe_checkout_routes.py -v --maxfail=10
.venv\Scripts\python.exe -m pytest ngo_homesuite/db/test_backup_restore_drill.py -v --maxfail=10
.venv\Scripts\python.exe -m pytest ngo_homesuite/db/test_cron_safe_rotate_db_key.py -v --maxfail=10
```

Outcome:

- `68 passed` (security lane)
- `24 passed` (tenant lane)
- `25 passed` (observability lane)
- `22 passed` (mobile/public lane)
- `2 passed` (backup/restore drill lane)
- `3 passed` (key-rotation drill lane)
- non-strict release evidence validation passes (`python tools/verify_release_evidence_bundle.py`)
- strict release evidence validation currently fails only on `external_pentest_signoff` pending status
...
| Legacy fallback surfaces active | Can reintroduce old or weaker behavior under failure scenarios. Runtime risk has been reduced by bridging legacy Flask audit writes to canonical `security_audit_events`, but full fallback retirement/governance is still required. |
